...
public class EventSerializationTests
{
    ...
    [Fact]
    public void StreamTitleSetEvent_Serializes_WithCorrectFieldNames()
    {
        ...
        doc.TryGetProperty("eventType", out _).Should().BeTrue();
        doc.TryGetProperty("source", out _).Should().BeTrue();
        doc.TryGetProperty("timestamp", out _).Should().BeTrue();
        doc.TryGetProperty("location", out _).Should().BeTrue();
        doc.TryGetProperty("traceId", out _).Should().BeTrue();
        doc.TryGetProperty("spanId", out _).Should().BeTrue();
        doc.TryGetProperty("parentSpanId", out _).Should().BeTrue();
        doc.TryGetProperty("data", out var data).Should().BeTrue();

        data.TryGetProperty("title", out _).Should().BeTrue("data.title must be present");
        data.TryGetProperty("targetPlatform", out _).Should().BeTrue("data.targetPlatform must be present");
        data.TryGetProperty("channelsUpdated", out _).Should().BeTrue("data.channelsUpdated must be present");
        data.TryGetProperty("channelsFailed", out _).Should().BeTrue("data.channelsFailed must be present");

        data.GetProperty("title").GetString().Should().Be("Sunday Liturgy");
        data.GetProperty("targetPlatform").GetString().Should().Be("youtube");
        data.GetProperty("channelsUpdated").GetInt32().Should().Be(2);
        data.GetProperty("channelsFailed").GetInt32().Should().Be(1);
    }

    [Fact]
    public void StreamTitleSetEvent_DefaultValues_AreCorrect()
    {
        var evt = new StreamTitleSetEvent();

        evt.EventType.Should().Be("StreamTitleSet");
        evt.Source.Should().Be("stream-title-service");
    }

    [Fact]
    public void StreamTitleFailedEvent_Serializes_WithCorrectFieldNames()
    {
        var evt = new StreamTitleFailedEvent
        {
            EventType = "StreamTitleFailed",
            Source = "stream-title-service",
            Timestamp = DateTimeOffset.UtcNow,
            Location = "us-east-1",
        ...
        data.TryGetProperty("title", out _).Should().BeTrue("data.title must be present");
        data.TryGetProperty("targetPlatform", out _).Should().BeTrue("data.targetPlatform must be present");
        data.TryGetProperty("error", out _).Should().BeTrue("data.error must be present");
        data.TryGetProperty("channelsUpdated", out _).Should().BeTrue("data.channelsUpdated must be present");
        data.TryGetProperty("channelsAttempted", out _).Should().BeTrue("data.channelsAttempted must be present");

        data.GetProperty("title").GetString().Should().Be("Sunday Liturgy");
        data.GetProperty("targetPlatform").GetString().Should().Be("youtube");
        data.GetProperty("error").GetString().Should().Be("API quota exceeded");
        data.GetProperty("channelsUpdated").GetInt32().Should().Be(0);
        data.GetProperty("channelsAttempted").GetInt32().Should().Be(3);
    }
